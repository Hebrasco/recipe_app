...
struct BreakfastFruitsView: View {
    ...
    @State var recipes: [Recipe] = Recipes.getRecipes().filter{$0.secondaryCategory.contains("Obst")}
    ...
    var body: some View {
        ...
        .onAppear(perform: {
            self.recipes = Recipes.getRecipes().filter{$0.secondaryCategory.contains("Obst")}
        })
